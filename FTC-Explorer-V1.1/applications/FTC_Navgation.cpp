...
{
	accDeadband(10,12,10);
	velDeadband(0.0022,0.0023,0.0022);
	posDeadband(0,0,0);
...
void FTC_Navgation::update(void)
{
	...
	/*
	static Vector3f v_sum;
	static Vector3f vs[50];
	static int v_cnt = 0;
	if(v_cnt<50){
		v_sum -= vs[v_cnt];
		v_sum += velocity;
	...
	static Vector3f a_sum;
	static Vector3f as[50];
	static int a_cnt = 0;
	if(a_cnt<50){
		a_sum -= as[a_cnt];
		a_sum += accel;
	...
	velocity.x = applyDeadband_float(velocity.x, velDeadband.x);
	velocity.y = applyDeadband_float(velocity.y, velDeadband.y);
	velocity.z = applyDeadband_float(velocity.z, velDeadband.z);
