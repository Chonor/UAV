...
{
	static Vector3f gyro_deg, acc_deg, gyro_rk2, gyro_last;
	static Matrix3f dcm_gyro;
	
	//积分角速度，得到二阶龙哥库塔累加项
	gyro_rk2 = (gyro_last + gyro) * (0.5f * deltaT);
	gyro_last = gyro;
	
	//迭代旋转测量向量
	dcm_gyro.from_euler(gyro_rk2);
	grav = dcm_gyro * grav;
	dir = dcm_gyro * dir;
	if(30 < accRatio && accRatio < 150)
		grav = CF_1st(grav, acc, ftc.factor.gyro_cf);
	else if(acc.z > 6000 && ftc.f.ARMED && fc.flag_fly == 1 && fc.flag_throw == 1) {
		fc.autoUp = acc.z / 6;
		fc.autoUp = constrain_int32(fc.autoUp, -1, 1800);
		
	}
	
	//通过测量向量得到姿态
	grav.get_rollpitch(angle);
	dir.get_yaw(angle);
}
